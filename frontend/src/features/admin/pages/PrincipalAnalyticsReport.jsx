...
const PrincipalAnalyticsReport = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { user } = useAuth();
  const { program, branch, batch, regulation, semester } = location.state || {};
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [expandedSubject, setExpandedSubject] = useState(null);

  if (!program || !branch || !batch || !regulation || !semester) {
  ...
  const getRoleHoverBorder = () => {
    if (user?.adminRole === 'principal') return 'hover:border-orange-500 hover:bg-orange-50';
    if (user?.adminRole === 'vice-principal') return 'hover:border-teal-500 hover:bg-teal-50';
    return 'hover:border-purple-500 hover:bg-purple-50';
  };

  // 20 questions template (same as student feedback questions)
  ...
  // Generate random ratings for each question
  ...
  const subjectPerformance = [
    { id: 1, subjectCode: `${branch.name}401`, subjectName: 'Data Structures', faculty: 'Dr. A. Kumar', avgRating: 4.5, responses: 48, type: 'Theory' },
    { id: 2, subjectCode: `${branch.name}402`, subjectName: 'Database Management Systems', faculty: 'Dr. B. Sharma', avgRating: 4.3, responses: 48, type: 'Theory' },
    { id: 3, subjectCode: `${branch.name}403`, subjectName: 'Operating Systems', faculty: 'Dr. C. Reddy', avgRating: 4.4, responses: 48, type: 'Theory' },
    { id: 4, subjectCode: `${branch.name}404`, subjectName: 'Computer Networks', faculty: 'Dr. D. Patel', avgRating: 4.2, responses: 48, type: 'Theory' },
    { id: 5, subjectCode: `${branch.name}405`, subjectName: 'Software Engineering', faculty: 'Dr. E. Singh', avgRating: 4.6, responses: 48, type: 'Theory' },
    { id: 6, subjectCode: `${branch.name}406`, subjectName: 'DBMS Lab', faculty: 'Prof. F. Gupta', avgRating: 4.4, responses: 48, type: 'Lab' },
    { id: 7, subjectCode: `${branch.name}407`, subjectName: 'OS Lab', faculty: 'Prof. G. Rao', avgRating: 4.3, responses: 48, type: 'Lab' },
    { id: 8, subjectCode: `${branch.name}408`, subjectName: 'Machine Learning', faculty: 'Dr. H. Verma', avgRating: 4.5, responses: 48, type: 'Elective' },
    { id: 9, subjectCode: `${branch.name}409`, subjectName: 'Web Technologies', faculty: 'Prof. I. Khan', avgRating: 4.2, responses: 48, type: 'Minor' },
    { id: 10, subjectCode: `${branch.name}410`, subjectName: 'Cyber Security', faculty: 'Dr. J. Mehta', avgRating: 4.7, responses: 48, type: 'Honors' }
  ].map(subject => ({
    ...subject,
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 lg:p-8 pt-16 lg:pt-8">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-6 lg:mb-8">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            <p className="text-sm lg:text-base text-gray-500 mt-2">{getRoleDisplayName()} - Detailed analysis of student feedback</p>
          </div>
          <div className="flex gap-3">
            <button
              onClick={() => navigate('/admin/analytics')}
              className="flex items-center gap-2 px-4 lg:px-5 py-2.5 lg:py-3 bg-white border-2 border-gray-200 rounded-xl hover:border-gray-400 hover:bg-gray-50 transition-all"
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 lg:px-5 py-2.5 lg:py-3 bg-white border-2 border-gray-200 rounded-xl ${getRoleHoverBorder()} transition-all disabled:opacity-50`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 lg:px-5 py-2.5 lg:py-3 bg-gradient-to-r ${getRoleGradient()} text-white rounded-xl hover:shadow-lg transition-all`}
            >
      ...
      {/* Breadcrumb */}
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-6 lg:mb-8 flex-wrap">
        <button onClick={() => navigate('/admin/analytics')} className={`hover:${getRoleTextColor()}`}>Analytics</button>
        ...
        <ChevronRight className="h-4 w-4" />
        <span className={`font-semibold ${getRoleTextColor()}`}>Report</span>
      </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`bg-gradient-to-r ${getRoleBgLight()} border ${getRoleBorderColor()} rounded-2xl p-5 lg:p-6 mb-6 lg:mb-8`}>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 lg:gap-6">
          <div>
            ...
            <p className="text-base lg:text-lg font-bold text-gray-900">{program.name}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Branch</p>
            <p className="text-base lg:text-lg font-bold text-gray-900">{branch.name}</p>
          </div>
          <div>
                  ...
                  >
                    <div className="p-3 sm:p-4">
                      ...
                      <div className="flex items-center gap-2 mb-3 sm:mb-4">
                        <BarChart3 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-orange-600" />
...
export default PrincipalAnalyticsReport;
